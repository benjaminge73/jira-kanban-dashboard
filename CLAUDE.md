...
- **Admin auth**: active iff `ADMIN_PASSWORD` is set (demo stays zero-config). Gate = `(protected)/layout.tsx` redirect + per-action session check in jh/settings writes. Live without password shows a warning in the admin header. The session cookie is `httpOnly` always and `Secure` whenever `NODE_ENV=production` (`sessionCookieOptions()` in `lib/auth/token.ts`) — no opt-out, so a non-HTTPS production deploy is a misconfiguration to fix, not a supported mode.
...
- CI (`.github/workflows/ci.yml`) runs on every push/PR to `main`: `npm ci` → lint → `tsc --noEmit` → test → build. Must stay green before merging.
